...
bool BitmapClass::UpdateBuffers(int positionX, int positionY)
{
	...
	if((size_changed_flag == false) && (positionX == m_previousPosX) && (positionY == m_previousPosY))
	{
		return true;
	}
...
void BitmapClass::SetSize (int size){
...
void BitmapClass::SetSize (int sizeX, int sizeY){
...
void BitmapClass::GetSize (int* size){

	size[0] = m_bitmapWidth;
	size[1] = m_bitmapHeight;
	return;
}