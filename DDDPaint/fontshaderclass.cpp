...
bool FontShaderClass::Initialize(ID3D11Device* device, HWND hwnd)
{
	...
	result = InitializeShader(device, hwnd, L"../DDDPaint/font.fx");
	if(!result)
	{
		return false;
	}
...
void FontShaderClass::Render(ID3D11DeviceContext* device, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix,
			     D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, D3DXVECTOR4 pixelColor, bool alphaEnabled)
{
	// Set the shader parameters that it will use for rendering.
	SetShaderParameters(worldMatrix, viewMatrix, projectionMatrix, texture, pixelColor, alphaEnabled);
...
bool FontShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* filename)
{
	...
	result = D3DX11CompileFromFile( filename, NULL, NULL, NULL, "fx_5_0", D3D10_SHADER_ENABLE_STRICTNESS, NULL, NULL, &effectBlob, &errorMessage, NULL );
	if(FAILED(result))
	{
	...
	if(FAILED(result))
	{
		MessageBox(hwnd, filename, L"Create effect fail", MB_OK);
	...
	m_alphaEnabledPtr = m_effect->GetVariableByName("useAlpha")->AsScalar();
...
void FontShaderClass::SetShaderParameters(D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix,
					  ID3D11ShaderResourceView* texture, D3DXVECTOR4 pixelColor, bool alphaEnabled)
{
	...
	m_alphaEnabledPtr->SetBool(alphaEnabled);
